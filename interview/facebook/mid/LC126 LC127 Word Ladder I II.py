            ...
            for cur in q:
                for i in range(len(cur)):
                    for c in ascii_lowercase: 
                        ...
                        if newW in visited or newW not in wordList:
                            continue
                        if newW == endWord:
                            return level
                        visited.add(newW)
...
"""
        top solution
        this is far better than the first one, take 386ms, first take 2:08, 
        this use the Bidirectional search, far better than normal bfs
"""
...
#assume that the word is long and wordlist is not very big
class Solution(object):
    # beginWord vs endWord is one different Letter and endWord has not been used
    def filter_condition(self, beginWord, endWord, q):
        if endWord in q:
            return False
        count = 0
    ...
    def ladderLength(self, beginWord, endWord, wordList):
        ...
        while index < len(q):
            ...
            for _ in range(size):
                cur = q[index]
                index += 1
                for word in filter(lambda a: self.filter_condition(cur, a, q), wordList):
                    if word == endWord:
                        return level
                    q.append(word)
...
#2头夹的：
# 一个hashmap，如果是从beginWord变过来的就是+1
# 如果是从endWord变过来的就是-1
from collections import deque
from collections import defaultdict
from string import ascii_lowercase
            ...
            for i in range(size):
                ...
                for i in range(len(cur)):
                    for c in ascii_lowercase: 
                        new = cur[:i] + c + cur[i+1:]
                        if new in record:
                            if record[cur]*record[new] < 0 :
                                return abs(record[cur]-record[new])
                        elif new in wordList and new not in record:
                            q.append(new)
                            if record[cur] > 0:
                                record[new] = record[cur]+1
                            else:
                                record[new] = record[cur]-1
...
s = Solution()
print s.ladderLength("sand", "acne", ["hot","dot","dog","lot","log","cog"])
